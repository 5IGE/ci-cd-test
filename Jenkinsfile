pipeline {
    ...
    parameters {
        ...
        string(name: 'SERVER_FQDN',
               defaultValue: 'ec2-3-121-186-210.eu-central-1.compute.amazonaws.com',
    ...
    stages {
        ...
        stage('Base installation') {
            ...
            steps {
                ...
                sh "ssh -o StrictHostKeyChecking=no -i ${env.SERVER_LOGIN} ${params.SSH_USER}@${params.SERVER_FQDN} 'sudo wget -O /etc/nginx/sites-available/default https://raw.githubusercontent.com/5IGE/ci-cd-test/k8s/nginx.conf'"
            ...
            steps {
                script {
                    ...
                    if (params.MONGODB == 'REDEPLOY_MONGODB') {
                        sh "ssh -o StrictHostKeyChecking=no -i ${env.SERVER_LOGIN} ${params.SSH_USER}@${params.SERVER_FQDN} 'kubectl apply -f https://raw.githubusercontent.com/5IGE/ci-cd-test/k8s/mongo.yaml'"
                        sleep 90
                    ...
                    sh "ssh -o StrictHostKeyChecking=no -i ${env.SERVER_LOGIN} ${params.SSH_USER}@${params.SERVER_FQDN} 'kubectl apply -f https://raw.githubusercontent.com/5IGE/ci-cd-test/k8s/rocketchat.yaml'"
                    sleep 90
